...
export default function IntroDisplay({ intro, isLoading }: { intro: Intro | null; isLoading: boolean }) {
  ...
  return (
    <div className="w-full max-w-[800px] px-4 mx-auto">
      <Card className={`${raleway.className} border-none shadow-sm`}>
        <CardContent className="p-4 sm:p-6">
          <div className="flex flex-col items-center sm:flex-row sm:items-start gap-4 sm:gap-6">
            ...
            <div className="flex-grow space-y-3 sm:space-y-4 text-center sm:text-left">
              <div>
                ...
                <p className="text-sm sm:text-base text-gray-600 mt-1">{intro.title}</p>
              </div>
              <p className="text-xs sm:text-sm text-gray-600 leading-relaxed">{intro.summary}</p>
              
              <div className="flex flex-wrap justify-center sm:justify-start gap-3 sm:gap-4 pt-3 sm:pt-4">
                {intro.contactEmail && (
                  <a 
                    href={`mailto:${intro.contactEmail}`}
                    className="inline-flex items-center gap-1.5 text-[11px] sm:text-xs text-gray-600 hover:text-black transition-colors"
                  >
